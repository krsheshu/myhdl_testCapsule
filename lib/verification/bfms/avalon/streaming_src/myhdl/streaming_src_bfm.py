...
class StreamingSrcBfm():
    ...
    def __init__(self, validPattern = 0xf0f0, dataFIFO = None, av_src_pars = None ):
        ...
        if (dataFIFO == None):
            raise ValueError("Pattern list for {} not passed !".format(__class__))  
        else:
            self.dataFIFO  = datFIFO
        
        """ Interfaces  """
        if (av_src_pars == None):
            raise ValueError("Avalon Streaming Src Interface Pars for {} not passed !".format(__class__))  
        else:
    ...
    def top( reset, clk, av_src ):  
        ...
        circular_patternQueue=Signal(intbv(valid_i_pattern)[self.PATTERNBITS:])
        
        @always(clk.posedge, reset.posedge)
        def vldPattern_shifter():
            """ Circular Queue for valid pattern """
            if enShift == 1:
                circular_patternQueue.next[self.PATTERNBITS:] = concat(circular_pattern_queue[self.PATTERNBITS-1:], circular_pattern_queue[self.PATTERNBITS-1])
